...
func (vn *LocalNode) init() {
	// Generate an ID
	fmt.Println("Entered LocalNode.Init function ")
	vn.genId()
	// Set our host
	...
	rpc.Register(vn)
	vn.schedule()
	//fmt.Println("My node Initialized as ",vn)
	go startRPCServer(vn.Host)
	fmt.Println("Exited LocalNode.Init func ")
...
func (vn *LocalNode) FindCoordinator(key []byte, reply *SNode) error {
	...
	//fmt.Println("Inside FindCoordinator function")
	...
	//fmt.Println("Exited FindCoordinator func")
	return nil
}
...
func (vn *LocalNode) FindCoordinatorWithFinger(key []byte, reply *([]*SNode)) error {
	...
	fmt.Println("Entered FindCoordinatorWithFinger func")
	...
	if betweenRightIncl(vn.Predecessor.Id, vn.Id, key){	
		...
		return nil
			...
			fmt.Println("Match else's if block")
			var res []*SNode
			err:=RPC_caller(vn.Host,"FindSuccessors","",&(res))
			checkError(err)
			...
			fmt.Println("Inside else's else-if block ")
			var res []*SNode
			err:=RPC_caller(vn.Successors[0].Host,"FindSuccessors","",&(res))
			checkError(err)

			copy(res[1:],res[0:])
			res[0].Host=vn.Successors[0].Host
			res[1].Id=vn.Successors[0].Id
			reply=&res
			return nil
	}
	...
	for {
		...
		closest := cp.Next()
		fmt.Println("closest node found : ",closest)
		if closest == nil {
			break
		}
		...
		//res, err := vn.ring.transport.FindSuccessors(closest, key)
		if err == nil {
			reply = &res
			return nil
		} else {
	...
	fmt.Println("Exited FindCoordinatorWithFinger")
...
func (vn *LocalNode) FindSuccessors(key string, succ_List *([]*SNode)) error{
	...
	//fmt.Println("Entered FindSuccessors ")
...
func (vn *LocalNode) Ping1(args interface{},reply *LocalNode) error{
	...
	//reply.Successors[0].Id = vn.Successors[0].Id
	//reply.Successors[0].Host = vn.Successors[0].Host 
	fmt.Println("Reply inside Ping - 1 ", reply)
	return nil
}
...
func (vn *LocalNode) Notify(maybe_pred *SNode, succ_List *([]*SNode)) error {
	...
	if vn.Predecessor!=nil{
		//fmt.Println("not equals nil condition")
...
func (vn *LocalNode) SkipSuccessor(s *SNode,reply *string) error {
	...
	if convert(vn.Successors[0].Id) == convert((*s).Id) {
		...
		/*vn.stabilize()
		fmt.Println("Value of My Successor after stabilize : ",vn.Successors[0].Host)
		fmt.Println("---------------------------------------------------------------------------")
		*/
	}
...
func (vn *LocalNode) stabilize() {
	//fmt.Println("Stabilize Called wt vn as : ")
	//fmt.Println(vn)
	//fmt.Println("----------------------------------------------------------------")
	//fmt.Println("")
	// Clear the timer
...
func (vn *LocalNode) checkNewSuccessor() error {
	...
	//fmt.Println("checkNewSuccessor Entered")

CHECK_NEW_SUC:
	/*fmt.Println("My entry point Successors list : ")
	...
	//fmt.Println("Value of maybe_succ",maybe_suc)
	...
	if err != nil {
		...
		if known > 1 {
			for i := 0; i < known; i++ {
				reply:=""
				//fmt.Println("Trying to ping ",vn.Successors[0].Host)
				err:=RPC_caller(vn.Successors[0].Host,"Ping",nil,&reply)
				
				//fmt.Println("Erroorrrr isss :  ",err)
				checkError(err)
				//fmt.Println("Reply for ping call",reply)
				if reply!="active"{
				...
				// Don't eliminate the last successor we know of
					...
					/*
					fmt.Println("My Updated Successors list here : ")
					for i, sc := range vn.Successors {
						fmt.Println("Successor [",i,"] : ",sc)
					}
					*/
		...
		fmt.Println("checkNewSuccessor Exited")
		return err
	}
	...
	if maybe_suc.Host!="" && between(vn.Id, succ.Id, maybe_suc.Id) {
		...
		//fmt.Println("Inside between case n maybe_suc :  ",maybe_suc.Host)
		rep:=""
		err:=RPC_caller(maybe_suc.Host,"Ping",nil,&rep)
		//TODO :::::::: change RPC_Caller to return error
...
/*
	else if maybe_suc.Host!="" && bytes.Compare(vn.Id,succ.Id)==0 {

		//Removed this condition from here : && bytes.Compare(vn.Id,maybe_suc.Id)==1
		//fmt.Println("Inside II between case ")
		//fmt.Println("maybe_suc :  ",maybe_suc )
		rep:=""
		err:=RPC_caller(maybe_suc.Host,"Ping",nil,&rep)
		//TODO :::::::: change RPC_Caller to return error
	...
	//	fmt.Println("vn at last of II between case wt succ_list containing only :   ")
	//	fmt.Println(vn.Successors[0])
	}
*/
	//fmt.Println("AT exit of checkNewSuccessors vn :  ", vn.Successors[0])
	return nil
...
func (vn *LocalNode) notifySuccessor() error {
	...
	//fmt.Println("Inside notifySuccessor wt vn's succ_List as  ")
	//fmt.Println(vn.Successors[0])
	
	succ := vn.Successors[0]
	//fmt.Println("notifySuccessor Entered wt successor as :  ",succ)
	...
	//fmt.Println("Exiting notifySuccessor")
...
func (vn *LocalNode) fixFingerTable() error {
	...
	fmt.Println("Entered fixFingerTable ")
	...
	fmt.Println("Exited fixFingerTable")
	return nil
}
...
func (vn *LocalNode) checkPredecessor() error {
	...
	//fmt.Println(vn.Predecessor)
	//fmt.Println("checkPredecessor Entered")
	// Check predecessor
	//s:=new(SNode)
	//fmt.Println(s)
	...
	//fmt.Println("vn before leaving checkPredecessor : ",vn)
	return nil
}
...
func (vn *LocalNode) knownSuccessors() (successors int) {
	//fmt.Println("Inside Known Successors")
	for i := 0; i < len(vn.Successors); i++ {
		if vn.Successors[i] != nil {
			//fmt.Println("Successor : ",i,"  --  ",vn.Successors[i].Host)
			successors = i + 1
		}
	}
	return
...
func (vn *LocalNode) leave() error {
	...
	//if vn.timer.Stop(){
		...
		if(reply=="successful"){
			...
			fmt.Println("SkipSuccessor returned successfully")
			...
			checkError(err2)
			fmt.Println("ClearPredecessor returned successfully")
